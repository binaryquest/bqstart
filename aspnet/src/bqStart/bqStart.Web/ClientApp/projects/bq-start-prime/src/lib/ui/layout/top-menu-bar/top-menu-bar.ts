...
export class TopMenuBar extends BaseMenu {
  ...
  mapMenuDataToPrimeMenu(menu: MenuData): MenuItem {
    ...
    if (menu.childMenus !== undefined) {
      childMenu = menu.childMenus
        .map((md) => this.mapMenuDataToPrimeMenu(md))
        .filter((x) => x.visible);
    }
    return {
      label: menu.label,
